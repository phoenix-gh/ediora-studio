# Ediora Image Upload via MCP

After generating a cover image, it needs a public URL to embed in X posts. MK hosts images on Ediora's upload endpoint.

## Normal Flow (MCP tools registered)

When Hermes Agent starts with the `ediora` MCP server connected, the tool `mcp_ediora_upload_image_from_base64` is available:
...
The Ediora MCP uses StreamableHTTP transport. Session IDs are:
...
- Base64 encoding adds ~33% overhead — for images >7.5 MB the base64 string will hit tool argument size limits. For very large images, use `upload_image_from_url` instead (upload to a temp host first, then let Ediora fetch it).
