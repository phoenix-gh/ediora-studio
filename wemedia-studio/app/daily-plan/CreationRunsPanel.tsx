    ...
    <div className="grid gap-2">
      ...
      {runs.map(run => {
        ...
        const outputs = Array.isArray(run.detail.outputs)
          ? run.detail.outputs as Array<{ draft_id?: number; plan_item_id?: number }>
          : []
          ...
          <div className="mt-3 space-y-3 border-t pt-3 text-xs">
            ...
            <div className="flex flex-wrap gap-3">
              {outputs.map((item, index) => item.draft_id
                ...
                : item.plan_item_id ? <span key={index}>计划项 #{item.plan_item_id}</span> : null)}
            </div>
          </div>
        </details>
      })}
