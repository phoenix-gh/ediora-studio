...
type Directory = { id: number; name: string }
const defaults: DailyCreationRuleInput = { name: '', asset_type: 'article', directory: '', output_type: 'x_short_post', target_count: 10, execution_mode: 'recurring', scheduled_date: null, scheduled_time: '09:00', timezone: 'Asia/Shanghai', lookback_days: 7, delivery_mode: 'drafts', account_id: null, instructions: '', enabled: true }

export function CreationRuleDialog({ open, directories, initial, onClose, onSubmit }: { open: boolean; directories: Directory[]; initial?: DailyCreationRule | null; onClose: () => void; onSubmit: (input: DailyCreationRuleInput) => void | Promise<void> }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label={initial ? '编辑创作规则' : '新建创作规则'} className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4">
    <form noValidate className="w-full max-w-xl space-y-4 rounded-2xl border bg-background p-5 shadow-xl" onSubmit={event => { event.preventDefault(); if (!value.directory) { setError('请选择素材目录'); return } if (!value.name.trim()) { setError('请输入规则名称'); return } if (value.execution_mode === 'once' && !value.scheduled_date) { setError('请选择执行日期'); return } setError(''); void onSubmit(value) }}>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="space-y-1 text-sm">素材目录<select aria-label="素材目录" value={value.directory} onChange={e => set('directory', e.target.value)} className="w-full rounded-lg border px-3 py-2"><option value="">请选择</option>{directories.map(item => <option key={item.id} value={item.name}>{item.name}</option>)}</select></label>
