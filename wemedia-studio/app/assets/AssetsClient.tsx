...
export function AssetsClient({ initialAssets }: { initialAssets: CreativeAsset[] }) {
  ...
  async function addArticle() { const content = window.prompt('原始内容'); if (!content?.trim()) return; const url = window.prompt('来源 URL（可留空）') ?? ''; const created = await createCreativeAsset({ asset_type: 'article', media_kind: '', title: '', content, url, media_type: '', filename: '', directory, tags: [] }); setAssets(items => [created, ...items]); setSelectedId(created.id) }
  async function editArticle(item: CreativeAsset) { const content = window.prompt('原始内容', item.content); if (!content?.trim()) return; const url = window.prompt('来源 URL（可留空）', item.url) ?? ''; const updated = await updateCreativeAsset(item.id, { content, url }); setAssets(items => items.map(value => value.id === item.id ? updated : value)) }
  async function removeArticle(item: CreativeAsset) { if (!window.confirm('删除这条原始素材？')) return; await deleteCreativeAsset(item.id); setAssets(items => items.filter(value => value.id !== item.id)); setSelectedId(null) }
  async function configureTopicRule() { if (!directory) return; const subscriptions = await listXSubscriptions(); const choices = subscriptions.map(item => `${item.id}: ${item.label}`).join('\n'); const rawId = window.prompt(`选择 X 订阅 ID：\n${choices}`); const subscriptionId = Number(rawId); if (!Number.isSafeInteger(subscriptionId) || subscriptionId <= 0) return; const rawKeywords = window.prompt('主题关键词（逗号分隔，可留空）') ?? ''; const created = await createTopicSourceRule({ subscription_id: subscriptionId, directory, keywords: rawKeywords.split(/[,，]/).map(item => item.trim()).filter(Boolean) }); setTopicRules(items => [created, ...items]) }
  async function importTopicSources() { const rules = topicRules.filter(item => item.directory === directory && item.enabled); if (!rules.length) { window.alert('请先为当前主题目录配置 X 入库规则。'); return } const now = Date.now(); await Promise.all(rules.map(rule => createJob({ flow: 'topic_source', title: `${directory}：X 主题素材甄选`, input: { rule_id: rule.id }, idempotency_key: `topic-source:${rule.id}:${now}` }))); window.alert(`已提交 ${rules.length} 个 AI 甄选任务，可在“任务中心”查看进度。`) }
  async function showDailyCandidates() { if (!directory) { window.alert('请先选择一个主题目录。'); return } const result = await selectDailyArticleCandidates(directory); setDailyCandidates(result.assets); if (!result.assets.length) window.alert('当前主题没有尚未选用的素材。') }
