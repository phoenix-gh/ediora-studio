...
vi.mock('./asset-image-import-plugin', () => ({
...
describe('AssetVisualMarkdownEditor', () => {
  ...
  it('pastes webpage structure and applies ordered per-image results', async () => {
    ...
    render(
      <AssetVisualMarkdownEditor documentKey={8} onChange={vi.fn()} value="" />,
    )
  ...
  it('uploads a pasted image file and inserts its relative local URL', async () => {
    ...
    render(
      <AssetVisualMarkdownEditor documentKey={9} onChange={vi.fn()} value="" />,
    )
    const editor = await screen.findByRole('textbox', { name: '可视化 Markdown 编辑器' })
  ...
  it('shows an explicit retry when editor initialization fails', async () => {
    ...
    render(
      <AssetVisualMarkdownEditor documentKey={10} onChange={vi.fn()} value="# 保留原文" />,
    )
  ...
  it('ignores a remote-image completion after the document changes', async () => {
    ...
    const { rerender } = render(
      <AssetVisualMarkdownEditor documentKey={11} onChange={vi.fn()} value="" />,
    )
    const editor = await screen.findByRole('textbox', { name: '可视化 Markdown 编辑器' })
    pasteClipboard(editor, {
    ...
    rerender(
      <AssetVisualMarkdownEditor documentKey={12} onChange={vi.fn()} value="# 下一篇" />,
    )
    request.resolve([{
