...
- Display `未命名文章` when an article has no title; never derive the visible list title from its body.
...
Article rows will use compact `px-4 py-3` spacing. The first line contains only the truncated title, and the second line contains the muted update time. Existing selected and hover states remain unchanged so density changes do not alter navigation feedback.
...
- article rows omit body content and the `文章` label, use compact spacing, and show `未命名文章` instead of body-derived fallback text;
