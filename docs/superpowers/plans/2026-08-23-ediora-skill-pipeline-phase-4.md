...
- Chat/Job frontend vertical gate: the Phase 1–4 focused set covering ChatClient, Skill picker, Pipeline card, Job hook, API routes/clients, first-party bindings, worker, and pipeline worker — 10 files, 59 tests passed.
- Backend pipeline/recovery gate: `tests/test_pipeline_service.py` — 15 passed; `tests/test_pipeline_runner.py tests/test_agent_execution_pipeline.py tests/test_execution_artifacts.py` — 14 passed; `tests/test_job_reconciliation.py tests/test_jobs_router.py` — 46 passed. The final combined migration/service/runner/reconciliation/router run passed 72 tests.
