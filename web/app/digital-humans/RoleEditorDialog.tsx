...
export function RoleEditorDialog({
  ...
  const [lookPrompt, setLookPrompt] = useState(role?.look_prompt ?? '')
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {role
                ? provider === 'comfyui'
                  ? '修改定妆说明、人像或环境后会重新生成定妆图。'
                  : '更换人物形象或声音后会重新处理对应资源。'
                : '选择渲染后端。HeyGen 需要录音克隆声音；ComfyUI 用生图模型生成定妆图。'}
            </DialogDescription>
          </DialogHeader>
          <FieldGroup>
            <Field>
            ...
            {provider === 'comfyui' ? (
              <Field>
                <FieldLabel htmlFor="role-look-prompt">定妆说明</FieldLabel>
                <Textarea
                  id="role-look-prompt"
                  value={lookPrompt}
                  onChange={event => setLookPrompt(event.target.value)}
                  placeholder="例如：坐在桌前，双手自然放在键盘上"
                />
                <FieldDescription>
                  人像和环境由参考图锁定。这里只改姿态、构图和动作；留空则默认中景站姿。
                </FieldDescription>
              </Field>
            ) : null}
            {role?.look ? (
              <Field>
                ...
                />
                <FieldDescription>
                  由生图模型根据人像和环境生成，生成口播时作为参考图。
                </FieldDescription>
              </Field>
            ) : role?.provider === 'comfyui' && role.status === 'processing' ? (
              <Field>
                <FieldLabel>定妆图</FieldLabel>
                <FieldDescription>正在生成，完成后会显示在这里。</FieldDescription>
              </Field>
            ) : null}
            <Button
              onClick={handleSave}
