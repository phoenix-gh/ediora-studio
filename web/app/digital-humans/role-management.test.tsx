...
describe('digital-human role creation', () => {
  ...
  it('lets a ComfyUI role edit look staging and saves it', async () => {
    mocks.updateDigitalHuman.mockResolvedValue({ id: 8, name: '林晓' })
    ...
    fireEvent.change(screen.getByLabelText('定妆说明'), {
      target: { value: '坐在桌前' },
    })
    fireEvent.click(screen.getByRole('button', { name: '保存角色' }))

    await waitFor(() => {
      expect(mocks.updateDigitalHuman).toHaveBeenCalledWith(8, expect.objectContaining({
        look_prompt: '坐在桌前',
      }))
    })
  })
