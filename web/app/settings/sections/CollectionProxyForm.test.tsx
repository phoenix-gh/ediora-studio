...
describe('CollectionProxyForm', () => {
  ...
  it('explains collection-only scope and covered collection sources', () => {
    render(<CollectionProxyForm settings={makeSettings()} onSaved={vi.fn()} />)

    expect(screen.getByText(/只作用于采集通道/)).toBeVisible()
    expect(screen.getByText(/不会写入进程级/)).toBeVisible()
