    ...
    >
      <DialogHeader className="shrink-0">
        <div className="flex items-center gap-2">
          ...
          {open && sessionId !== null && isRunning && <Loader2 data-testid="trajectory-running-indicator" role="status" aria-label="运行中" className="size-4 animate-spin text-info" />}
        </div>
        <DialogDescription>按 Turn、Message、Step 和 Tool 查看本会话 Agent 运行轨迹。</DialogDescription>
      </DialogHeader>
      <div className="min-h-0 flex-1 overflow-hidden">
        {sessionId !== null && <AgentTrajectoryPanel scope={{ session_id: sessionId }} open={open} developerModeEnabled={developerModeEnabled} showHeader={false} onRunningChange={setIsRunning} />}
      </div>
    </DialogContent>
  </Dialog>
}
