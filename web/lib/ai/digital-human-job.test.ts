...
import {
  buildLookStillPrompt,
  LOOK_STILL_DEFAULT_DIRECTION,
  LOOK_STILL_IDENTITY,
...
describe('digital-human durable jobs', () => {
  it('keeps identity constraints and lets staging be edited', () => {
    expect(buildLookStillPrompt()).toContain(LOOK_STILL_IDENTITY)
    expect(buildLookStillPrompt()).toContain(LOOK_STILL_DEFAULT_DIRECTION)
    expect(buildLookStillPrompt('坐在桌前，双手放在键盘上')).toContain(
      '坐在桌前，双手放在键盘上',
    )
    expect(buildLookStillPrompt('坐在桌前')).not.toContain(
      LOOK_STILL_DEFAULT_DIRECTION,
    )
  })
  ...
  it('generates a look still with portrait and environment references', async () => {
    const api = jobApi('digital_human_setup', { digital_human_id: 7 })
    api.getRoleContext.mockResolvedValue({
      id: 7,
      name: '林晓',
    ...
    api.fetchLocalAsset.mockImplementation(async (_url: string, fallback: {
      filename: string
    }) => ({
      bytes: new Uint8Array(fallback.filename.startsWith('portrait') ? [1] : [2]),
      mediaType: fallback.filename.startsWith('portrait') ? 'image/png' : 'image/jpeg',
      filename: fallback.filename,
    }))
    const generateLook = vi.fn().mockResolvedValue({
      look_asset_id: 44,
      url: '/api/uploads/look.png',
    })
    ...
    expect(api.composeLook).not.toHaveBeenCalled()
    expect(generateLook).toHaveBeenCalledWith({
      jobId: 41,
      roleName: '林晓',
      lookPrompt: '坐在桌前',
      portrait: {
