...
type RoleContext = {
  ...
  portrait: LocalAsset
  default_environment?: LocalAsset | null
...
export const LOOK_STILL_IDENTITY = [
  'Photorealistic 16:9 shot of the same person from the first reference photo,',
  'wearing the same clothes, in the environment from the second reference photo.',
  'Preserve face identity and outfit. No text, no logo, no watermark, no extra people.',
].join(' ')

export const LOOK_STILL_DEFAULT_DIRECTION = (
  'Medium shot, standing naturally, facing the camera.'
)

export function buildLookStillPrompt(direction = '') {
  const staging = direction.trim() || LOOK_STILL_DEFAULT_DIRECTION
  return `${LOOK_STILL_IDENTITY} Staging: ${staging}`
}

export const LOOK_STILL_PROMPT = buildLookStillPrompt()

export type GenerateLookInput = {
  jobId: number
  roleName: string
  lookPrompt?: string
  portrait: DownloadedAsset
  environment: DownloadedAsset
}
...
export type DigitalHumanJobDeps = {
  ...
  generateLook?(input: GenerateLookInput): Promise<{ look_asset_id: number; url: string }>
}
...
export async function generateLookStill(input: GenerateLookInput) {
  const generated = await generateAndSaveImage({
    jobId: input.jobId,
    prompt: buildLookStillPrompt(input.lookPrompt),
    title: `${input.roleName} 定妆图`,
    directory: '数字人资产',
    referenceImages: [
      { bytes: input.portrait.bytes, mediaType: input.portrait.mediaType },
      { bytes: input.environment.bytes, mediaType: input.environment.mediaType },
    ],
    size: '1536x1024',
  })
  return {
    look_asset_id: generated.asset_id,
    url: generated.asset_url,
  }
}
...
async function runComfyUISetupJob(
  ...
  const generateLook = deps.generateLook ?? generateLookStill
  try {
    const look = await runStep(
      ...
      async () => {
        ...
        if (!context.default_environment) {
          throw new Error('数字人缺少环境图')
        }
        ...
        const environment = await deps.api.fetchLocalAsset(
          context.default_environment.url,
          context.default_environment,
        )
        const composed = await generateLook({
          jobId,
          roleName: context.name,
          lookPrompt: context.look_prompt ?? '',
          portrait,
          environment,
        })
...
export async function runDigitalHumanSetupJob(
  ...
  if ((context.provider || 'heygen') === 'comfyui') {
    await runComfyUISetupJob(
      ...
      roleId,
      context,
      providedDeps ?? { api: defaultApi, sleep, generateLook: generateLookStill },
    )
    return
  }
