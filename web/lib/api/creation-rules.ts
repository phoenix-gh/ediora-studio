...
export type DailyCreationRule = { id: number; name: string; prompt: string; asset_type: 'article' | 'media'; directory: string; directories?: string[]; output_type: 'x_short_post'; target_count: number; execution_mode: 'once' | 'recurring'; scheduled_date: string | null; scheduled_time: string; timezone: string; lookback_days: number; delivery_mode: 'drafts'; account_id: string | null; instructions: string; skill_mode: 'auto' | 'manual'; skill_name: string | null; enabled: boolean; deleted_at: string | null; last_run_at: string | null; next_run_at: string | null; created_at: string; updated_at: string }
export type DailyCreationRuleInput = Omit<DailyCreationRule, 'id' | 'created_at' | 'updated_at' | 'deleted_at' | 'last_run_at' | 'next_run_at' | 'directories'> & { directories: string[] }
